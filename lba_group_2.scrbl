...
and non-compensatory decision making strategies, screening and eliminating possible alternatives and making trade-offs. When compensatory decision making strategies are used, positive and negative qualities of all possible alternatives are evaluated such that positive qualities compensate for negative ones. ;; cite this
...
@figure**[
        "fig:lbamodelled"
        @elem{Linear Ballistic Acummulator Described}
...
In the paper, stimulus features such as the ease-of-pronunciation for non-words and the natural occurence frequency for words were manipulated. They found reliable changes in response accuracy, and the shapes of RT distributions for correct and incorrect responses @~cite[plt-tr2].
...
The model's prediction were able match this data very well as the worst error is a 0.053-s over-prediction for the 90% quantile estimate from incorrect responses to low frequency words. 
...
As the model is simplfiied, it uses less parameters but it got relatively accurate results which implies that this model, although it isn't as complicated, can still accomplish what it is meant to do.
...
Participants were given two conditions: speed-emphasis instructions (which they were told to prioritize their speed rather than accuracy) and then accuracy-emphasis instructions in which they were told not to make too many mistakes. 
...
While this is only a simple binary decision model, and does not demonstrate the versatility a linear ballistic accumulator
...
As the LBA is a simplication of Usher and Clelland's (2001) leaky competitive accumulator, the leaky competitive accumulator has received support from arguments regarding the evidence of neural plausibility.
...
The authors outline 2 defenses against that criticism.
...
such as ion flows, membrane potentials and action potentials. The LBA model adds another further step but the approximiations shouldn't be seen as harmfull towards the scientific progress.
...
that only has one response accumulator and does not consider between-trial variability in the starting points of evidence accumulation. This model is similar to LBA however it's limitations
mean that the LATER model provides an incomplete account of behavior data, specifically of error responses. The point is that if the neurophysiological community can accept models that omit within-trial variability,
